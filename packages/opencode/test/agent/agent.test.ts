...
test("custom agent config overrides native agent properties", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(vibe?.model?.providerID).toBe("anthropic")
      expect(vibe?.model?.modelID).toBe("claude-3")
      expect(vibe?.description).toBe("Custom vibe agent")
      expect(vibe?.temperature).toBe(0.7)
      expect(vibe?.color).toBe("#FF0000")
      expect(vibe?.native).toBe(true)
    },
  })
})
...
test("agent permission config merges with defaults", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("bash", "rm -rf *", claw!.permission).action).toBe("deny")
      // Edit still allowed
      expect(evalPerm(claw, "edit")).toBe("allow")
    },
  })
})
...
test("global permission config applies to all agents", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(claw).toBeDefined()
      expect(evalPerm(claw, "bash")).toBe("deny")
    },
  })
})
...
test("agent steps/maxSteps config sets steps property", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(claw?.steps).toBe(50)
      expect(adaptive?.steps).toBe(100)
    },
  })
})
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        claw: { prompt: "Custom system prompt" },
      },
    },
  })
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(claw?.prompt).toBe("Custom system prompt")
    },
  })
})
...
test("unknown agent properties are placed into options", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(claw?.options.random_property).toBe("hello")
      expect(claw?.options.another_random).toBe(123)
    },
  })
})
...
test("agent options merge correctly", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(claw?.options.custom_option).toBe(true)
      expect(claw?.options.another_option).toBe("value")
    },
  })
})
...
test("default permission includes doom_loop and external_directory as ask", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const vibe = await Agent.get("vibe")
      expect(evalPerm(vibe, "doom_loop")).toBe("ask")
      expect(evalPerm(vibe, "external_directory")).toBe("ask")
    },
  })
})
...
test("webfetch is allowed by default", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const vibe = await Agent.get("vibe")
      expect(evalPerm(vibe, "webfetch")).toBe("allow")
    },
  })
})
...
test("legacy tools config converts to permissions", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(evalPerm(claw, "bash")).toBe("deny")
      expect(evalPerm(claw, "read")).toBe("deny")
    },
  })
})
...
test("legacy tools config maps write/edit/patch/multiedit to edit permission", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(evalPerm(claw, "edit")).toBe("deny")
    },
  })
})
...
test("Truncate.GLOB is allowed even when user denies external_directory globally", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(PermissionNext.evaluate("external_directory", Truncate.GLOB, claw!.permission).action).toBe("allow")
      expect(PermissionNext.evaluate("external_directory", Truncate.DIR, claw!.permission).action).toBe("deny")
      expect(PermissionNext.evaluate("external_directory", "/some/other/path", claw!.permission).action).toBe("deny")
    },
  })
})
...
test("explicit Truncate.GLOB deny is respected", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const claw = await Agent.get("claw")
      expect(PermissionNext.evaluate("external_directory", Truncate.GLOB, claw!.permission).action).toBe("deny")
      expect(PermissionNext.evaluate("external_directory", Truncate.DIR, claw!.permission).action).toBe("deny")
    },
  })
})
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(PermissionNext.evaluate("external_directory", target, claw!.permission).action).toBe("allow")
      },
    })
  } finally {
...
test("defaultAgent returns socratic when no default_agent config", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
