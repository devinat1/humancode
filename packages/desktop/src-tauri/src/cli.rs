...
const CLI_BINARY_NAME: &str = "humancode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("humancode-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.opencode/bin/humancode\"".to_string(),
...
pub fn serve(
    ...
    .expect("Failed to spawn humancode");
