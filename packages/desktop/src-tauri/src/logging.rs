...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("humancode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("humancode_lib=debug,humancode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("humancode_lib=info,humancode_desktop=info,sidecar=info")
        }
    });
