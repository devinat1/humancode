...
:::tip
Use the socratic agent to walk through unfamiliar code one breakpoint at a time. It guides you to discover behavior rather than narrating it.
:::
...
Primary agents are the main assistants you interact with directly. You can cycle through them using the **Tab** key, or your configured `switch_agent` keybind. These agents handle your main conversation. Tool access is configured via permissions — for example, Pair is advice-only with no edit access, while Claw has all tools enabled.

:::tip
You can use the **Tab** key to switch between primary agents during a session.
:::

OpenCode comes with five built-in primary agents: **Pair**, **Socratic**, **Vibe**, **Claw**, and **Adaptive**. We'll look at these below.
...
OpenCode comes with three built-in subagents, **General**, **Explore**, and **Review**. We'll look at these below.
...
A pair-programming advisor. Pair never writes or edits code — it suggests approaches, explains trade-offs, and answers questions. Use this when you want to talk through a problem without the agent reaching for the keyboard.
...
The **default** primary agent. Guides you to discover how code works through one-question-at-a-time Socratic dialog, paired with exactly one live debugger breakpoint per question. Read-only with respect to source code. Use socratic when you want to understand a function, trace why something returns the value it does, or learn how a flow executes — without the agent telling you the answer.
...
A multi-task manager. Parses your request into discrete tasks, confirms the plan, then works through each with self-review and a review subagent. Use vibe for moderate multi-step work — new features, multi-file changes, or anything with two or more distinct steps.
...
Fully autonomous. No confirmations, self-reviews against quality standards, runs tests. Use claw for straightforward mechanical tasks where the scope is clear — renames, config changes, boilerplate, simple bug fixes.
...
Adaptive agent that transitions between modes based on task complexity and outcomes. Uses a classifier to decide whether to behave like pair, socratic, vibe, or claw given the current request.
...
Hidden subagent that reviews code against configured quality standards. Invoked by `vibe` and `claw` after task completion, or manually via `@review`.
...
- **Socratic agent**: Walk through unfamiliar code, debug tricky behavior, learn how a flow executes
- **Pair agent**: Discuss approaches and trade-offs before writing any code
- **Claw agent**: Run mechanical tasks autonomously
- **Vibe agent**: Multi-step feature work with checkpoints
- **Adaptive agent**: Auto-adjust between styles based on task complexity
