...
The mode system is now deprecated in favor of [Agents](/docs/agents) — see that page for the current primary agents (Pair, Socratic, Vibe, Claw, Adaptive). The configuration syntax below still works for backward compatibility, but new functionality is added to the agents API.
...
```json title="opencode.json" {4-14}
{
  ...
  "mode": {
    "readonly": {
      "prompt": "{file:./prompts/readonly.txt}",
      "tools": {
        "write": false,
        "edit": false,
        "bash": false,
...
- **Read-only mode**: Analysis and exploration without making changes
